...
export interface Loop {
  ...
  settings: {
    ...
    radius?: number; // for neighborhood loops
    centerPoint?: {
      lat: number;
      lng: number;
      address: string;
...
// Sample data for different loop types
...
// Sample loops for different types
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [currentLoop, setCurrentLoop] = useState<Loop | null>(sampleLoops[1]);
  const [userLoops, setUserLoops] = useState<Loop[]>(sampleLoops);

  // Initialize services
  useEffect(() => {
    if (currentUser && currentLoop) {
      // Initialize WebSocket connection
      websocketService.connect(currentUser.id, currentLoop.id);
      ...
      websocketService.on('realtime_update', (update) => {
  ...
  const createLoop = (name: string, description: string, type: LoopType) => {
    ...
    websocketService.sendUpdate({
      type: 'member_joined',
      data: { loopId: newLoop.id, user: currentUser },
      loopId: newLoop.id,
  ...
  const joinLoop = (code: string): boolean => {
    // Find existing loop by code
    const existingLoop = sampleLoops.find(loop => loop.code === code);
    if (existingLoop && currentUser) {
      ...
      if (!existingLoop.members.find(member => member.id === currentUser.id)) {
        ...
        websocketService.sendUpdate({
          type: 'member_joined',
          data: { loopId: updatedLoop.id, user: currentUser },
  ...
  const addItem = (itemData: Omit<Item, 'id' | 'createdAt'>) => {
    ...
    // Show notification to other members
    notificationService.notifyItemBorrowed(newItem.title, currentUser?.name || 'Someone');
  };
  ...
  return (
    <AppContext.Provider value={{
      ...
      setCurrentLoop: (loop) => {
        setCurrentLoop(loop);
        // Update the loop in userLoops as well
